...
const emojiIconTypes = new Set(Object.values(emojiMap));

export const getEmojiIndexByIconType = (iconType: string): number | undefined => {
  const match = /^icon-a-(\d+)$/.exec(iconType);
  ...
  return emojiIconTypes.has(iconType) ? index : undefined;
};

export const getEmojiIconTypeByIndex = (index?: number): string | undefined => {
  if (!index || !Number.isFinite(index)) {
    return undefined;
  }

  const iconType = `icon-a-${index}`;

  return emojiIconTypes.has(iconType) ? iconType : undefined;
};
