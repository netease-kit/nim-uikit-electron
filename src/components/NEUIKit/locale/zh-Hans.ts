// zn.js
export default {
  appText: "云信IM H5",
  messageText: "消息",
  contactText: "通讯录",
  mineText: "我的",
  logoutText: "退出登录",
  commsEaseText: "关于云信",
  welcomeText: "欢迎使用云信",
  logoutConfirmText: "确定退出登录吗?",
  avatarText: "头像",
  updateAvatarText: "修改头像",
  accountText: "账号",
  name: "昵称",
  genderText: "性别",
  remarkText: "备注名",
  mobile: "手机",
  email: "邮箱",
  sign: "签名",

  // 托盘菜单相关
  trayShowWindowText: "显示窗口",
  trayAboutText: "关于",
  trayQuitText: "退出",
  trayUnreadMessagesText: "未读消息",
  trayNoUnreadMessagesText: "无未读消息",
  trayTooltipText: "云信 IM",

  // 关于页面相关
  aboutVersionText: "版本",
  aboutDescriptionText: "应用描述",
  aboutDeveloperText: "开发团队",
  aboutContactText: "联系我们",
  aboutWebsiteText: "官方网站",
  aboutBackText: "返回",
  man: "男",
  woman: "女",
  unknow: "未知",
  birthText: "生日",
  FriendPageText: "好友名片",
  startConversationText: "发起单聊",
  addText: "添加",
  removeText: "移除",
  addTeamManagerSuccessText: "添加管理员成功",
  addTeamManagerFailText: "添加管理员失败",
  confirmRemoveText: "是否移除？",
  removeManagerExplain: "移除后该成员将无管理权限",
  removeMemberExplain: "将移除该成员",
  removeSuccessText: "移除成功",
  removeFailText: "移除失败",
  teamIdText: "群ID",
  userNotInTeam: "该用户不在群里",
  noTeamManager: "暂无群管理员",
  noTeamMember: "暂无成员",
  applyFriendSuccessText: "好友申请已发送",
  applyFriendFailText: "发送申请失败",
  addFriendText: "添加好友",
  addMemberText: "添加成员",
  addChatMemberText: "添加聊天成员",
  addFriendFailText: "添加好友失败",
  deleteFriendText: "删除好友",
  deleteFriendConfirmText: "是否确认删除好友",
  deleteFriendSuccessText: "删除好友成功",
  deleteFriendFailText: "删除好友失败",
  setBlackFailText: "加入黑名单失败",
  removeBlackFailText: "解除黑名单失败",
  removeBlackSuccessText: "解除黑名单成功",
  addPermissionText: "请在系统设置页添加相关权限",
  FailAvatarText: "上传头像失败",
  chooseDefaultImage: "选择默认图标",
  chatButtonText: "去聊天",
  gotoChatFailText: "去聊天失败",
  chatWithFriendText: "聊天",
  okText: "确定",
  confirmText: "确定",
  updateText: "修改",
  cancelText: "取消",
  friendSelectText: "选择好友",
  telErrorText: "手机格式不正确",
  emailErrorText: "邮箱格式不正确",
  createButtonText: "创建",
  createP2PText: "发起单聊",
  createTeamText: "创建群聊",
  addTeamMemberText: "添加群成员",
  addTeamMemberSuccessText: "添加成员成功",
  addTeamMemberFailText: "添加成员失败",
  addTeamMemberWaitingVerifyText: "等待验证中",
  teamTitlePlaceholder: "请输入群名称",
  placeHolderText: "请输入内容",
  maxSelectedText: "最多只能选择200位好友",
  createTeamSuccessText: "创建群组成功",
  createTeamFailedText: "创建群组失败",
  createDiscussionText: "创建讨论组",
  discussionMemberText: "讨论组成员",
  createDiscussionSuccessText: "创建讨论组成功",
  createDiscussionFailedText: "创建讨论组失败",
  leaveDiscussionTitle: "退出讨论组",
  leaveDiscussionConfirmText: "是否确认退出该讨论组",
  leaveDiscussionSuccessText: "已成功退出此讨论组",
  leaveDiscussionFailedText: "退出此讨论组失败",
  PersonalPageText: "个人名片",
  deleteSessionText: "删除会话",
  sessionMuteText: "开启消息提醒",
  sessionMuteFailText: "开启消息提醒失败",
  sessionUnMuteFailText: "关闭消息提醒失败",
  updateBitConfigMaskSuccess: "更新消息免打扰成功",
  updateBitConfigMaskFailed: "更新消息免打扰失败",
  teamDoNotDisturbText: "群免打扰",
  discussionDoNotDisturbText: "讨论组免打扰",
  stickTopText: "聊天置顶",
  deleteStickTopText: "取消置顶",
  addStickTopText: "置顶消息",
  deleteSessionFailText: "删除会话失败",
  deleteStickTopFailText: "取消置顶失败",
  addStickTopFailText: "置顶消息失败",
  muteSessionText: "开启免打扰",
  unmuteSessionText: "取消免打扰",
  teamMemberText: "群成员",
  mergeForwardText: "合并转发",
  pleaseSelectMsg: "请选择消息",
  teamManagerText: "群管理",
  teamManagerSettingText: "管理管理员",
  updateTeamInfoText: "谁可以编辑群信息",
  updateTeamInviteText: "谁可以邀请新成员",
  updateTeamAtText: "谁可以@所有人",
  transformTeam: "转让群主",
  transformTeamContent: "确认转让群主给",
  doubt: "吗？",
  transformTeamSuccessText: "转让群主成功",
  transformTeamFailedText: "转让群主失败",
  transformTeamText: "选择转让成员",
  teamBannedText: "群禁言",
  sensitiveText: "触发敏感词",
  saveText: "保存",
  setText: "设置",
  failText: "失败",
  saveSuccessText: "保存成功",
  saveFailedText: "保存失败",
  searchTeamPlaceholder: "请输入群名称",
  nickInTeamEmptyText: "请输入群昵称",
  mobilePlaceholderText: "请输入手机号",
  nickPlaceholderText: "请输入昵称",
  signPlaceholderText: "请输入签名",
  emailPlaceholderText: "请输入邮箱",
  joinTeamFailedText: "加入群组失败",
  joinTeamApplySentText: "入群申请已发送",
  joinTeamLimitText: "群组人数达到上限",
  teamMuteText: "当前群聊禁言中",

  teamTitle: "群名称",
  teamIntro: "群介绍",
  teamAvatar: "群头像",
  networkError: "当前网络错误",
  searchFailText: "查找失败",
  offlineText: "当前网络不可用，请检查你当前网络设置。",
  connectingText: "当前IM连接已断开，正在重连...",
  teamMenuText: "我的群组",
  myRobotsText: "我的机器人",
  robotEmptyText: "暂无机器人",
  robotEmptyTipText: "点击右上角创建机器人",
  robotLimitReachedText: "机器人数量已达上限",
  getRobotListFailedText: "获取机器人列表失败",
  createRobotText: "创建机器人",
  robotAvatarText: "机器人头像",
  robotNameText: "机器人名称",
  robotNamePlaceholder: "请输入机器人名称",
  robotNameRequiredText: "请输入机器人名称",
  uploadImageText: "上传图片",
  createRobotSuccessText: "创建机器人成功",
  createRobotFailedText: "创建机器人失败",
  robotAccountIdText: "机器人账号",
  viewRobotConfigText: "查看配置串",
  robotConfigText: "机器人配置串",
  robotConfigWarningText: "配置串包含敏感信息，请妥善保管。",
  copyFullConfigText: "复制完整配置",
  refreshRobotTokenText: "刷新 Token",
  refreshTokenConfirmText: "刷新后旧 Token 将失效，是否继续？",
  refreshTokenSuccessText: "刷新 Token 成功",
  refreshTokenFailedText: "刷新 Token 失败",
  deleteRobotText: "删除机器人",
  deleteRobotConfirmText: "是否确认删除该机器人？",
  deleteRobotSuccessText: "删除机器人成功",
  deleteRobotFailedText: "删除机器人失败",
  saveRobotSuccessText: "保存机器人成功",
  saveRobotFailedText: "保存机器人失败",
  viewConfigFailedText: "获取配置失败",
  teamChooseText: "我的群聊",
  chooseText: "选择",
  friendSelect: "请选择联系人",
  pleaseSelectMember: "请选择成员",
  teamMemebrSelect: "人员选择",
  teamManagerSelect: "人员管理",
  getHistoryMsgFailedText: "获取失败",
  deleteText: "删除",
  recallText: "撤回",
  copyText: "复制",
  forwardText: "转发",
  moreText: "更多",
  quickCommentText: "快捷评论",
  quickCommentFailedText: "快捷评论失败",
  multiSelect: "多选",
  forwardMsgMax100Text: "合并转发限制100条消息",
  forwardMsgHasUnforwardableText: "存在不可转发的消息体",
  forwardHasUnforwardableConfirmText: "存在不可转发的消息体，是否去除后发送",
  exceptionDescription: "异常说明",
  chatHistoryText: "聊天记录",
  noHistoryText: "暂无聊天记录",
  mergeForwardDepthLimitText: "合并转发层级不能超过3层",
  messageOfText: "的消息",
  forwardSystemErrorText: "系统异常，转发失败",
  forwardComment: "留言",
  forwardSuccessText: "转发成功",
  forwardFailedText: "转发失败",
  getMessageFailed: "获取消息失败",
  replyText: "回复",
  replyNotFindText: "该消息已撤回或删除",
  replyDeletedText: "该消息已删除",
  replyRecalledText: "该消息已撤回",
  replyNotLoadedText: "该消息尚未加载",
  msgRecallTimeErrorText: "已超过时间无法撤回",
  enableCloudConversationText: "是否开启云端会话",
  enableCloudSearchText: "是否开启云端搜索",
  SwitchToEnglish: "切换语言为英文",
  copySuccessText: "复制成功",
  copyFailedText: "复制失败",
  tipText: "提示",
  selectImageText: "请选择图片文件",
  uikitVersion: "版本号",
  IMVersion: "IM版本号",
  productInfoText: "产品介绍",

  forwardToTeamText: "转发到群组",
  forwardToFriendText: "转发到个人",
  sessionRecordText: "的会话记录",
  chatInputPlaceHolder: "说点什么呢~",
  pinText: "标记",
  pinFailedText: "标记失败",
  unpinText: "取消标记",
  unpinFailedText: "取消标记失败",
  noPinListText: "暂无标记消息",
  collectionText: "收藏",
  deleteCollectionText: "删除收藏",
  addCollectionSuccessText: "已收藏",
  addCollectionFailedText: "添加收藏失败",
  deleteCollectionFailedText: "删除收藏失败",
  deleteCollectionConfirmText: "是否确认删除收藏？",
  noCollectionsText: "暂无收藏",
  you: "你",
  pinThisText: "标记了这条消息",
  delete: "删除此消息",
  recall: "撤回了一条消息",
  recall2: "此消息已撤回",
  recall3: "撤回此消息",
  recallMsgFailText: "撤回消息失败",
  reEditText: "重新编辑",
  loadingMoreText: "加载更多……",
  loadingText: "正在加载...",
  noMoreText: "没有更多了",
  deleteMsgConfirmText: "是否确认删除选中的消息？",
  deleteMsgMax50Text: "批量删除限制50条消息",
  deleteMsgSuccessText: "删除成功",
  deleteMsgFailText: "删除失败",
  deleteSuccessText: "删除成功",
  deleteFailedText: "删除失败",
  conversationEmptyText: "暂无会话",
  teamOwner: "群主",
  teamManager: "群管理员",
  manager: "管理员",
  teamEmptyText: "暂无群聊",
  updateTeamManagerSuccessText: "修改群管理员成功",
  updateTeamManagerFailText: "修改群管理员失败",
  blacklistEmptyText: "暂无黑名单",
  blacklistSubTitle: "你不会收到列表中任何联系人的消息",
  validEmptyText: "暂无验证消息",
  dismissTeamText: "解散群组",
  dismissTeamConfirmText: "是否确认解散该群组",
  leaveTeamTitle: "删除并退出",
  leaveTeamConfirmText: "是否确认退出该群组",
  leaveTeamSuccessText: "已成功退出此群",
  leaveTeamFailedText: "退出此群失败",
  dismissTeamSuccessText: "群解散成功",
  dismissTeamFailedText: "群解散失败",
  updateTeamSuccessText: "修改成功",
  updateTeamFailedText: "修改失败",
  teamTitleConfirmText: "群名称不能为空",
  teamIntroConfirmText: "群介绍不能为空",
  aliasConfirmText: "备注名不能为空",
  sendImageFailedText: "发送图片失败",
  sendVideoFailedText: "发送视频失败",
  sendAudioFailedText: "发送音频失败",
  sendFileFailedText: "发送文件失败",
  sendMsgFailedText: "发送消息失败",
  weekText: "周前",
  dayText: "天前",
  hourText: "小时前",
  minuteText: "分钟前",
  nowText: "刚刚",
  sendText: "发送",
  sendToText: "发送给",
  textMsgText: "文本消息",
  audioMsgText: "音频消息",
  videoMsgText: "视频消息",
  fileMsgText: "文件消息",
  callMsgText: "话单消息",
  geoMsgText: "地理位置消息",
  imgMsgText: "图片消息",
  notiMsgText: "通知消息",
  robotMsgText: "机器消息",
  tipMsgText: "提示消息",
  customMsgText: "自定义消息",
  unknownMsgText: "暂不支持消息体",
  noFriendText: "暂无好友",
  onDismissTeamText: "该群聊已解散",
  onRemoveTeamText: "您已离开群组",
  onRemoveDiscussionText: "您已离开讨论组",
  receiveText: "您收到了新消息",
  backToBottomText: "回到底部",
  selectSessionFailText: "选择会话失败",
  discussionText: "讨论组资料",

  noExistUser: "该用户不存在",
  enterAccount: "请输入账号",
  validMsgText: "消息中心",
  blacklistText: "黑名单",
  applyTeamText: "申请加入",
  acceptResultText: "已同意",
  rejectResultText: "已拒绝",
  rejectTeamInviteText: "拒绝了群邀请",
  beRejectResultText: "拒绝了好友申请",
  passResultText: "通过了好友申请",
  acceptedText: "已同意该申请",
  acceptFailedText: "同意该申请失败",
  rejectedText: "已拒绝该申请",
  rejectFailedText: "拒绝该申请失败",
  passFriendAskText: "我已经同意了你的申请，现在开始聊天吧~",
  applyFriendText: "添加您为好友",
  rejectText: "拒绝",
  acceptText: "同意",
  inviteTeamText: "邀请您加入",
  rejectInviteText: "拒绝了群邀请",
  rejectApplyText: "已拒绝群申请",
  acceptInviteText: "已同意群邀请",
  acceptApplyText: "已同意群申请",
  addBlacklist: "加入黑名单",
  removeBlacklist: "解除",
  teamAll: "所有人",
  sendFailWithInBlackText: "对方已将你拉黑，消息发送失败",
  sendFailWithDeleteText: "双方关系已解除，如需沟通，请申请",
  friendVerificationText: "好友验证",
  chooseMentionText: "选择提醒",
  someoneText: "有人",
  meText: "我",
  teamMutePlaceholder: "当前群主已经设置为群禁言",
  noPermission: "您暂无权限操作",
  muteAllTeamFailedText: "全员禁言失败",
  unmuteAllTeamFailedText: "解除全员禁言失败",
  friendEditPlaceholder: "请输入备注名",

  nickInTeam: "我在群里的昵称",
  viewMoreText: "查看更多",
  resendMsgFailText: "重发失败",
  audioBtnText: "按住说话",
  audioRemindText: "松开发送",
  audioErrorText: "录音失败，请检查是否开启权限",
  videoPlayText: "视频播放",
  updateTeamAvatar: "更新了群头像",
  updateTeamName: "更新群名称为",
  updateTeamIntro: "更新了群介绍",
  updateTeamInviteMode: "更新了群权限“邀请他人权限”为",
  updateTeamUpdateTeamMode: "更新了群权限“群资料修改权限”为",
  teamManagerEditInfoText: "谁可以修改资料",

  teamInfoText: "群资料",
  discussionTitleText: "讨论组",
  discussionAvatarText: "讨论组头像",
  discussionIdText: "讨论组ID",
  discussionTitle: "讨论组名称",
  searchDiscussionMemberPlaceholder: "搜索讨论组成员",
  updateAllowAt: '更新了"@所有人权限"为',
  updateTeamMute: '更新了"群禁言"为',
  updateTeamJoinModeOpen: "开启了入群审核",
  updateTeamJoinModeClose: "关闭了入群审核",
  updateTeamAgreeModeOpen: "开启了入群邀请需要同意",
  updateTeamAgreeModeClose: "关闭了入群邀请需要同意",
  onlyTeamOwner: "仅群主",
  teamOwnerAndManagerText: "群主和管理员",
  closeText: "关闭",
  openText: "开启",
  joinTeamText: "加入群组",
  joinTeamSuccessText: "加入群组成功",
  leaveTeamText: "离开了群组",
  beRemoveTeamText: "被移出群组",
  beAddTeamManagersText: "被任命为管理员",
  beRemoveTeamManagersText: "被移除管理员",
  newGroupOwnerText: "成为新群主",
  callDurationText: "通话时长",
  callCancelText: "已取消",
  callRejectedText: "已拒绝",
  callTimeoutText: "超时未接听",
  callBusyText: "忙线未接听",
  callFailedText: "呼叫失败",
  wxAppFileCopyText: "已自动复制网址，请在手机浏览器里粘贴该网址",
  shootText: "拍摄",
  albumText: "相册",
  voiceCallText: "语音通话",
  audioCallText: "语音通话",
  videoCallText: "视频通话",
  msgReadPageTitleText: "消息阅读状态",
  readText: "已读",
  unreadText: "未读",
  fileText: "文件",
  imgText: "图片",
  videoText: "视频",
  allUnReadText: "全部成员未读",
  allReadText: "全部成员已读",
  searchText: "请输入你要搜索的关键字",
  searchTitleText: "搜索",
  friendText: "好友",
  teamText: "群组",
  searchResultNullText: "该用户不存在",
  conversationSendFailText: "[发送失败]",
  conversationNotificationText: "[通知消息]",
  getForwardMessageFailed: "该消息已撤回或删除",
  getMergeForwardFailedText: "信息获取失败",
  fileMsgTitleText: "[文件消息]",
  collectionFromText: "来自",
  openUrlText: "已复制链接，请在浏览器打开链接",
  SwitchToEnglishText: "是否切换为英文",
  securityTipText:
    "仅用于体验云信产品功能，请勿轻信汇款、中奖等涉及钱款的信息，勿拨打陌生电话，谨防受骗。",
  strangerTipText: "当前不是您的好友，请注意保护个人隐私安全",
  sendToFriend: "发送给好友",
  sendToTeam: "发送给群聊",
  recentConversationText: "最近会话",
  searchButtonText: "查找",
  accountNotMatchText: "查无此账号",
  teamIdNotMatchText: "查无此群号",

  selectedText: "已选",
  personUnit: "人",
  teamIdPlaceholder: "请输入群组 ID",
  editNickInTeamText: "编辑我在群里的昵称",

  userOnlineText: "在线",
  userOfflineText: "离线",

  unblacklistText: "取消拉黑",
  blacklistFriendText: "拉黑好友",
  deleteFriendMenuText: "删除好友",
  setNicknamePlaceholder: "设置昵称",
  sendMessageText: "发送消息",
  aliasText: "备注",
  unknownText: "未知",
  blacklistSuccessText: "已拉黑",
  unblacklistSuccessText: "已取消拉黑",
  blacklistFailText: "拉黑失败",
  unblacklistFailText: "取消拉黑失败",
  saveAliasSuccessText: "备注保存成功",
  saveAliasFailedText: "保存备注失败",
  uploadLimitText: "文件/视频大小最大支持100Mb",
  uploadingText: "正在上传",
  uploadCancelText: "取消上传",
  uploadCancelledText: "上传已取消",
  pasteFileFailedText: "粘贴文件失败",

  searchNoResText: "暂无搜索结果",
  msgNetworkErrorText: "网络异常，发送失败",
  myFriendsText: "我的好友",
  searchTeamMemberPlaceholder: "搜索群成员",

  processedValidationMessageText: "该验证消息已被处理",
  teamNotExitsText: "群组不存在",
  teamMemberLimitText: "群成员数量已达上限",
  verifyMsgNotExitsText: "该验证消息已被处理",
  aleadyInTeamText: "已在群组",
  expiredText: "已过期",

  //demo层
  session: "会话",
  addressText: "通讯录",
  sdkVersionSelectionText: "sdk版本选择",
  p2pMsgVisibleModeText: "是否显示p2p消息已读未读",
  teamMsgVisibleModeText: "是否显示群消息已读未读",
  addFriendMode: "添加好友模式",
  yesText: "是",
  noText: "否",
  needVerifyText: "需要验证",
  notNeedVerifyText: "不需要验证",
  oppositeSide: "对方",
  settingText: "设置",
  callFailed: "呼叫失败",
  inCallText: "已在通话中，请结束通话后再呼叫",
  needMentionText: "是否需要@消息",
  teamManagerEnableText: "是否开启群管理员功能",
  languageText: "语言",
  zhText: "中文",
  enText: "English",

  // 窗口关闭行为相关
  closeWindowTitleText: "关闭窗口",
  closeWindowConfirmText: "选择关闭方式",
  minimizeToTrayText: "最小化到托盘",
  quitAppText: "退出应用",
  rememberChoiceText: "记住我的选择",
  closeActionSettingText: "关闭行为",
  closeActionDefaultText: "默认（每次询问）",
  closeActionMinimizeText: "最小化到托盘",
  closeActionQuitText: "退出应用",

  // 桌面通知相关
  newMessagesCount: "收到 {count} 条新消息",
  clickToView: "点击查看",
  desktopNotificationText: "桌面消息通知",
  desktopNotificationDesc: "收到新消息时显示系统通知",
  smsApiEnvText: "短信接口环境",
  prodEnvText: "正式环境",
  qaEnvText: "测试环境",

  aiStreamText: "数字人流式输出",
  teamJoinModeText: "入群审核",
  teamJoinModeExplainText: "开启后，成员入群时需要群主或管理同意",
  teamAgreeModeText: "入群邀请需同意",
  teamAgreeModeExplainText: "开启后，拉入入群，需被邀请人同意",
  teamJoinModeFreeText: "自由加入，无须验证",
  teamJoinModeNeedVerifyText: "需申请，群主或管理同意后加入",
  // teamAgreeModeText: "邀请入群时是否需要被邀请人的同意模式",
  needBeInvitedText: "需要被邀请人同意",
  notNeedBeInvitedText: "不需要被邀请人同意",

  // emoji 不能随便填，要用固定 key
  Laugh: "[大笑]",
  Happy: "[开心]",
  Sexy: "[色]",
  Cool: "[酷]",
  Mischievous: "[奸笑]",
  Kiss: "[亲]",
  Spit: "[伸舌头]",
  Squint: "[眯眼]",
  Cute: "[可爱]",
  Grimace: "[鬼脸]",
  Snicker: "[偷笑]",
  Joy: "[喜悦]",
  Ecstasy: "[狂喜]",
  Surprise: "[惊讶]",
  Tears: "[流泪]",
  Sweat: "[流汗]",
  Angle: "[天使]",
  Funny: "[笑哭]",
  Awkward: "[尴尬]",
  Thrill: "[惊恐]",
  Cry: "[大哭]",
  Fretting: "[烦躁]",
  Terrorist: "[恐怖]",
  Halo: "[两眼冒星]",
  Shame: "[害羞]",
  Sleep: "[睡着]",
  Tired: "[冒星]",
  Mask: "[口罩]",
  ok: "[OK]",
  AllRight: "[好吧]",
  Despise: "[鄙视]",
  Uncomfortable: "[难受]",
  Disdain: "[不屑]",
  ill: "[不舒服]",
  Mad: "[愤怒]",
  Ghost: "[鬼怪]",
  Angry: "[发怒]",
  Unhappy: "[不高兴]",
  Frown: "[皱眉]",
  Broken: "[心碎]",
  Beckoning: "[心动]",
  Ok: "[好的]",
  Low: "[低级]",
  Nice: "[赞]",
  Applause: "[鼓掌]",
  GoodJob: "[给力]",
  Hit: "[打你]",
  Please: "[阿弥陀佛]",
  Bye: "[拜拜]",
  First: "[第一]",
  Fist: "[拳头]",
  GiveMeFive: "[手掌]",
  Knife: "[剪刀]",
  Hi: "[招手]",
  No: "[不要]",
  Hold: "[举着]",
  Think: "[思考]",
  Pig: "[猪头]",
  NoListen: "[不听]",
  NoLook: "[不看]",
  NoWords: "[不说]",
  Monkey: "[猴子]",
  Bomb: "[炸弹]",
  Cloud: "[筋斗云]",
  Rocket: "[火箭]",
  Ambulance: "[救护车]",
  Poop: "[便便]",
  Eyes: "[眼睛]",
  // 附件下载相关
  loadingSourceImage: "正在加载原图...",
  downloadingVideo: "正在下载视频...",
  downloadFailed: "下载失败",

  // 错误码及提示内容
  // 通用错误码
  190001: "内部错误",
  190002: "非法状态",
  191001: "使用姿势错误",
  191002: "操作取消",
  191003: "回调失败",
  191004: "参数错误",
  191005: "请求超时",
  191006: "资源不存在",
  191007: "资源已存在",
  // 连接相关
  192001: "连接错误",
  192002: "连接超时",
  192003: "连接断开",
  192004: "协议超时",
  192005: "协议发送失败",
  192006: "请求失败",
  // 消息相关
  107319: "PIN 数量超限",
  107320: "PIN 消息不存在",
  107322: "重复 PIN",
  107327: "未开启 PIN 功能",
  189301: "收藏数量超限",
  189302: "收藏不存在",
  189449: "并发操作收藏失败",
  // 群相关
  109432: "没有权限",
  multiSelectText: "多选",

  // 搜索相关
  searchHistoryTitle: "搜索聊天记录",
  searchHistoryPlaceholder: "搜索聊天内容",
  enableCloudSearch: "搜索云端记录",
  searching: "搜索中...",
  searchResultsCount: "找到 {count} 条相关记录",
  noSearchResults: "暂无搜索结果",
  searchHistoryTip: "输入关键字搜索历史记录",
  searchError: "搜索失败，请重试",
  loadingMore: "正在加载更多...",
  noMoreData: "没有更多数据了",
  jumpToMessageError: "跳转消息失败，请重试",
  topicSubsessionTitle: "小信 AI · 子会话",
  topicSubsessionTag: "子会话",
  topicSubsessionSearchPlaceholder: "搜索子会话",
  topicSubsessionLoadFailed: "加载失败，点击重试",
  topicSubsessionEmptyText: "还没有会话，开始第一个吧",
  topicSubsessionNoSearchResult: "未找到相关子会话",
  topicSubsessionDefaultTitle: "新会话",
  topicSubsessionOfflineCreateTip: "当前离线，暂无法新建会话",
  topicSubsessionSelectPlaceholder: "请选择或新建一个子会话",
  topicSubsessionRenameText: "重命名",
  topicSubsessionRenamePlaceholder: "请输入子会话名称",
  topicSubsessionRenameFailed: "重命名失败",
  topicSubsessionDeleteConfirmText: "是否确认删除该子会话？",
  topicSubsessionRetryFirstMessageTip: "请先重发第一条消息",
  accountBannedText: "当前账号已被封禁",
  missingLoginConfigTitleText: "缺少登录配置",
  fixedLoginTitleText: "固定账号登录",
  fixedLoginFormDescText: "输入应用 ID（AppKey）、账号和登录令牌",
  fixedLoginMissingFieldsText: "请补全：{fields}",
  appKeyText: "应用 ID（AppKey）",
  appKeyPlaceholderText: "请输入应用 ID（AppKey）",
  accountPlaceholderText: "请输入账号",
  tokenText: "登录令牌",
  tokenPlaceholderText: "请输入登录令牌",
  showTokenText: "显示",
  hideTokenText: "隐藏",
  loginText: "登录",
  loginFailMsg: "登录失败",
};
