...
const allowAtAll = computed(() => {
  ...
  try {
    ...
    console.error("Failed to parse team server extension:", error);
  }
...
// 更新群禁言
const updateTeamMute = (teamMute: V2NIMTeamChatBannedMode) => {
...
// 发送消息失败提示
const handleSendMsgError = (errCode: number) => {
  switch (errCode) {
    case 102426:
...
const removeReplyMsg = () => {
  const cid = replyMsg.value?.conversationId;
  if (cid) {
    store?.msgStore.removeReplyMsgActive(cid);
  }
  isReplyMsg.value = false;
};
...
onMounted(() => {
  ...
  emitter.on(events.ON_REEDIT_MSG, (_msg) => {
    ...
    isReplyMsg.value = false;
    store?.msgStore.removeReplyMsgActive(msg.conversationId as string);
