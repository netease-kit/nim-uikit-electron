...
onMounted(() => {
  ...
  if (messageClientId && conversationId) {
    teamId.value =
      nim?.conversationIdUtil?.parseConversationTargetId(conversationId) || "";
    ...
    if (targetMsg) {
      ...
      store?.msgStore
        ...
        .then((res) => {
          readCount.value = res?.readReceipt?.readCount || 0;
          unReadCount.value = res?.readReceipt?.unreadCount || 0;
